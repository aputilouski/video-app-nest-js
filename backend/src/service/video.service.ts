...
export class VideoService {
  ...
  async createVideo(video: Record<string, unknown>): Promise<Video> {
    const newVideo = new this.videoModel(video);
    return newVideo.save();
  }

  async readVideo(id): Promise<any> {
    if (id.id) return this.videoModel.findOne({ _id: id.id }).populate('createdBy').exec();
  ...
  async streamVideo(id: string, response: Response, request: Request) {
    try {
      ...
      if (!data) throw new NotFoundException(null, 'VideoNotFound');
